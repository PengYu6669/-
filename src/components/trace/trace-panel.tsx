  ...
  return (
    <div className="flex flex-col h-full bg-card">
      ...
      <div className="px-4 py-3 border-b shrink-0">
        ...
        {/* 金额对比卡片 */}
        {record.invoiceDate != null && record.amountInclTax != null && (
          <div className="mt-2 p-2 rounded-lg bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 text-xs flex items-center gap-3">
            <span>📅 {String(record.invoiceDate).slice(0,10)}</span>
            <span className="font-bold text-base">¥{Number(record.amountInclTax).toLocaleString()}</span>
            <span className="text-muted-foreground">发票金额</span>
          </div>
        )}
        {(record as any).receiptDate != null && (record as any).amount != null && (
          <div className="mt-2 p-2 rounded-lg bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-800 text-xs flex items-center gap-3">
            <span>📅 {String((record as any).receiptDate).slice(0,10)}</span>
            <span className="font-bold text-base">¥{Number((record as any).amount).toLocaleString()}</span>
            <span className="text-muted-foreground">签收单金额</span>
          </div>
        )}
